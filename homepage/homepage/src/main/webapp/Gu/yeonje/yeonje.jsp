       	...
       	<div class ="text-center">
       		<ul class="pagination">
       			<c:if test="${pageMaker.prev}" >
					...
					<li><a href="./yeonje${pageMaker.makeQuery(pageMaker.startPage - 1)}">이전</a></li>
				</c:if>
				<c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage }" var="idx">
					<li <c:out value="${pageMaker.criteria.page == idx ? 'class=active' : ' ' }"/>>
						<%-- <a href="./listPaging?page=${idx}">${idx}</a>--%>
						<a href="./yeonje${pageMaker.makeQuery(idx)}">${idx}</a>
					</li>					
				</c:forEach>
				<c:if test="${pageMaker.next && pageMaker.endPage > 0}">
					<%--<li><a href="./listPaging?page=${pageMaker.endPage + 1}">다음</a></li> --%>
					<li><a href="./yeonje${pageMaker.makeQuery(pageMaker.endPage + 1)}">다음</a></li>
				</c:if>
       		</ul>
       	</div>
       </div>
