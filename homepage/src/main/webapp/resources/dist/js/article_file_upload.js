...
function getFileInfo(fullName) {
	...
	if(checkImageType(fullName)) {
		...
		//원본 이미지 요청 링크
        originalFileUrl = "../../board/file/display?fileName=" + fullName.substr(0, 12) + fullName.substr(14);

	} else {
		imgSrc = "../../resources/dist/upload/files/food.PNG"; // 파일 아이콘 이미지 링크
...
function getFiles(articleNo){
	$.getJSON("../../board/file/list/"+articleNo, function(list){
		if(list.lenght === 0){
			$(".uploadFileList").html("<span class='noAttach'>첨부파일이 없습니다.</span>");
		}
